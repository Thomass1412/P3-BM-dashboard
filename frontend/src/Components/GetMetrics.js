...
function Getmetric() {
  ...
  const handlemetricClick = (e) => {
    navigate("/metrics/"+(e.target.parentNode.getAttribute('id')))
  };
          ...
          <table className="min-w-full text-left text-xl font-light">
            <thead className="border-b text-xl dark:border-neutral-500">
              <tr className='bg-green-50'>
                ...
                <th scope="col" className="px-6 py-4">dependentmetricId</th>
              </tr>
            </thead>
            <tbody>
              {data.map((metric) => (
                <tr id={metric.id} className="py-4 even:bg-green-200 odd:bg-green-50 hover:bg-blend-darken hover:bg-gray-300" key={metric.id} onClick={handlemetricClick}>
                  <td className="whitespace-nowrap px-6 py-4">{metric.id}</td>
                  <td className="whitespace-nowrap px-6 py-4">{metric.name}</td>
                  <td className="whitespace-nowrap px-6 py-4">{metric.dependentIntegrationIds}</td>
                </tr>
              ))}
            </tbody>
          </table>
