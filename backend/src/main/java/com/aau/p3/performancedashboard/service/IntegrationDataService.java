...
public class IntegrationDataService {
	...
	/**
	 * Creates a new MongoDB collection with a JSON schema derived from the provided {@link CreateIntegrationRequest}.
	 * The schema is generated by calling {@link CreateIntegrationRequest#generateSchema()}.
	 * If the schema generation is successful, a new collection is created with the generated schema.
	 * If the schema generation fails, an error is logged and a {@link RuntimeException} is propagated.
	 *
	 * @param integrationRequest the {@link CreateIntegrationRequest} from which to generate the schema and collection name.
	 * @return a {@link Mono<String>} that emits the name of the created collection when the operation is successful.
	...
	public Mono<String> createCollection(CreateIntegrationRequest integrationRequest) {
		return Mono.fromCallable(integrationRequest::generateSchema)
				.doOnSuccess(s -> logger.info("Created schema: " + s.toString() + " for collection: " + integrationRequest.getName()))
				.doOnError(e -> logger.error("Error creating schema for collection: " + integrationRequest.getName(), e))
		...
		return integrationService.findById(integrationId)
				.flatMap(integration -> {
					if (!integration.getType().equals("internal")) {
						...
						// Convert the integration data request to a document ready to be inserted into the database
						Document document = IntegrationDataConverter.convertCreateIntegrationDataRequestToDocument(integrationId, integrationDataRequest);
						...
						return mongoOperations.getCollection(integration.getDataCollection())
								.flatMap(collection -> Mono.from(collection.insertOne(document))) // Insert the document into the collection
								.doOnSuccess(result -> logger.info("Successfully saved: " + document.toJson() + " to collection: " + integration.getDataCollection())) // Log the result
								.doOnError(error -> logger.error("Error saving: " + document.toJson() + " to collection: " + integration.getDataCollection(), error)) // Log the error
